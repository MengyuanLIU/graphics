...
/*绘制填充的矩形*/
void glColourSqure(int x, int y, int R, GLubyte color[3])
{/* 开始绘制曲线 */
	glBegin(GL_POLYGON);
	/* 设置颜色 */
	glColor3ub(color[0], color[1], color[2]);
...
void glWriteWord(int x, int y, char *str)
{
	int n = strlen(str);
	//设置要在屏幕上显示字符的起始位置  
	glRasterPos2i(x, y);
...
void display(void)
{
	...
	glWriteWord(-280, 260, "selected colour");
	glColorCircle(-180, 280, 10, nowColor);


	glColourSqure(240, 280, 10, startBtn);
	glWriteWord(230, 260, "start");
	glColourSqure(280, 280, 10, endBtn);
	glWriteWord(270,260,"end");
	glColourSqure(200, 280, 10, delBtn);
	glWriteWord(180, 260, "delete");
	glColourSqure(160, 280, 10, moveBtn);
	glWriteWord(145, 260, "move");
	glColourSqure(120, 280, 10, storeBtn);
	glWriteWord(100, 260, "store");
	glColourSqure(80, 280, 10, loadBtn);
	glWriteWord(60, 260, "load");
...
int main(int argc, char *argv[])
{
	...
	/* 创建窗口，同时为之命名 */
	glutCreateWindow("The Experiment 2 of Computer Graphics");
	/* 设置窗口清除颜色为白色 */
